...
def generate_buccino_reconstructions(gtr, plot_dir, unit_id, suffix, fresh_plots=True):
    """
    Generates the necessary axon reconstructions using the Buccino algorithm results.
    Plots axon reconstruction and velocity from the gtr object (GraphAxonTracking) produced by the Buccino algorithm.
    
    Parameters:
    - gtr: GraphAxonTracking object (output from compute_graph_propagation_velocity).
    - plot_dir: Directory to save the plots.
    - unit_id: Unit ID to label the plots.
    - suffix: Suffix for the file names.
    - fresh_plots: Whether to generate new plots or reuse old ones.
    """
    paths_files_generated = []
    
    # Generate Axon Reconstruction Heuristics
    try:
        generate_axon_reconstruction_heuristics(gtr, plot_dir, unit_id, suffix=f"_{suffix}", fresh_plots=fresh_plots, figsize=(20, 10))
        paths_files_generated.append(plot_dir / f"BUCCaxon_reconstruction_heuristics{suffix}.png")
    except Exception as e:
        if logger: 
    ...
    try:
        generate_axon_reconstruction_raw(gtr, plot_dir, unit_id, plot_dir, {}, suffix=f"_{suffix}", fresh_plots=fresh_plots, plot_full_template=False)
        generate_axon_reconstruction_raw(gtr, plot_dir, unit_id, plot_dir, {}, suffix=f"_{suffix}_full", fresh_plots=fresh_plots, plot_full_template=True)
        paths_files_generated.append(plot_dir / f"BUCCaxon_reconstruction_raw{suffix}.png")
        paths_files_generated.append(plot_dir / f"BUCCaxon_reconstruction_raw{suffix}_full.png")
    except Exception as e:
        if logger: 
    ...
    try:
        generate_axon_reconstruction_clean(gtr, plot_dir, unit_id, plot_dir, {}, suffix=f"_{suffix}", fresh_plots=fresh_plots, plot_full_template=False)
        generate_axon_reconstruction_clean(gtr, plot_dir, unit_id, plot_dir, {}, suffix=f"_{suffix}_full", fresh_plots=fresh_plots, plot_full_template=True)
        paths_files_generated.append(plot_dir / f"BUCCaxon_reconstruction_clean{suffix}.png")
        paths_files_generated.append(plot_dir / f"BUCCaxon_reconstruction_clean{suffix}_full.png")
    except Exception as e:
        if logger: 
    ...
    try:
        ...
        paths_files_generated.append(plot_dir / f"BUCCaxon_reconstruction_velocities{suffix}.png")
    except Exception as e:
        if logger: 
            logger.info(f"unit {unit_id}_{suffix} failed to generate axon_reconstruction_velocities, error: {e}")
        plt.close()

    return paths_files_generated
...
def approx_milos_tracking(template_dvdt, locations, params, plot_dir, fresh_plots, **kwargs):
    ...
    analysis_options = {'generate_animation': True}
    ...
    # try:
    #     # Generate Axon Reconstruction Heuristics
    #     generate_axon_reconstruction_heuristics(gtr9, plot_dir, unit_id, suffix=f"_{suffix}", fresh_plots=True, figsize=(20, 10))
    #     paths_files_generated.append(plot_dir / f"axon_reconstruction_heuristics{suffix}.png")
    # except Exception as e:
    #     if logger: 
    #         logger.info(f"unit {unit_id}_{suffix} failed to generate axon_reconstruction_heuristics, error: {e}")
    ...
    # # Generate Axon Reconstruction Raw
    # try:
    #     generate_axon_reconstruction_raw(gtr9, plot_dir, unit_id, plot_dir, successful_recons, suffix=f"_{suffix}", fresh_plots=True, plot_full_template=False)
    #     generate_axon_reconstruction_raw(gtr9, plot_dir, unit_id, plot_dir, successful_recons, suffix=f"_{suffix}_full", fresh_plots=True, plot_full_template=True)
    #     paths_files_generated.append(plot_dir / f"axon_reconstruction_raw{suffix}.png")
    #     paths_files_generated.append(plot_dir / f"axon_reconstruction_raw{suffix}_full.png")
    # except Exception as e:
    #     if logger: 
    ...
    # # Generate Axon Reconstruction Clean
    # try:
    #     generate_axon_reconstruction_clean(gtr9, plot_dir, unit_id, plot_dir, successful_recons, suffix=f"_{suffix}", fresh_plots=True, plot_full_template=False)
    #     generate_axon_reconstruction_clean(gtr9, plot_dir, unit_id, plot_dir, successful_recons, suffix=f"_{suffix}_full", fresh_plots=True, plot_full_template=True)
    #     paths_files_generated.append(plot_dir / f"axon_reconstruction_clean{suffix}.png")
    #     paths_files_generated.append(plot_dir / f"axon_reconstruction_clean{suffix}_full.png")
    # except Exception as e:
    #     if logger: 
    ...
    # # Generate Axon Reconstruction Velocities
    # try:
    #     generate_axon_reconstruction_velocities(gtr9, plot_dir, unit_id, suffix=f"_{suffix}")
    #     paths_files_generated.append(plot_dir / f"axon_reconstruction_velocities{suffix}.png")
    # except Exception as e:
    #     if logger: 
    #         logger.info(f"unit {unit_id}_{suffix} failed to generate axon_reconstruction_velocities, error: {e}")
